...
def get_exact_image_tokens(image_path, text_prompt):
    ...
    try:
        ...
        try:
            ...
            # If it succeeds, let's try to get the actual token count
            # by making a request with a very small max_tokens
            try:
                ...
                print("✅ Second request also succeeded")
                return None  # We can't get exact count this way
            except Exception as e2:
                error_msg2 = str(e2)
                print(f"Second request error: {error_msg2}")
                
                # Parse error message for token count
                if "length" in error_msg2 and "tokens" in error_msg2:
                    match = re.search(r'length (\d+)', error_msg2)
                    if match:
...
if exact_tokens:
    ...
    # Let's try a different approach - make a request with max_tokens=0
    print("\n🔍 Trying max_tokens=0 approach...")
    try:
        messages = [
            {
                "role": "user",
        ...
        response = client.chat.completions.create(
            model="Qwen/Qwen2.5-VL-3B-Instruct",
            messages=messages,
            max_tokens=0  # This should give us just the input token count
        )
        print("✅ max_tokens=0 request succeeded")
        
    except Exception as e:
        error_msg = str(e)
        print(f"max_tokens=0 error: {error_msg}")
        ...
        if "length" in error_msg and "tokens" in error_msg:
            ...
            if match:
                total_tokens = int(match.group(1))
                print(f"📊 Total input tokens (from max_tokens=0 error): {total_tokens}")
                ...
                # Check against model limit
                max_model_len = 1024
                available_for_output = max_model_len - total_tokens
                
                print(f"\n📏 Model context limit: {max_model_len} tokens")
                print(f"📊 Available for output: {available_for_output} tokens")
                
                if total_tokens > max_model_len:
                    print(f"❌ Input too large! Need {total_tokens} tokens but model only supports {max_model_len}")
                    print("💡 Solution: Increase --max-model-len when starting vLLM server")
                else:
                    print(f"✅ Input fits! Can generate up to {available_for_output} tokens")
